...
class PaymentController extends Controller
{
    ...
    public function showPaymentForm(Request $request)
    {
        ...
        if (!$id_pesanan) {
            ...
            return view('payment.form', ['message' => $page_data['message']]);
        } 
        ...
        // 2. Gunakan data dari objek $order
        $page_data = [
            'id_pesanan'        => $order->id_pesanan,
            'payment_success'   => $payment_success,
            'message'           => Session::get('status_message'),
            // Mengambil detail penerima dari tabel pesanan
            'name'              => old('name', $order->nama_penerima), 
            'phone'             => old('phone', $order->nomor_telepon),
            'address'           => old('address', $order->alamat_pengiriman),
            // Menggunakan total_harga dari DB
