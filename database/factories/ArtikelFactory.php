...
class ArtikelFactory extends Factory
{
    ...
    public function definition()
    {
        ...
        return [
            ...
            'gambar' => $randomArticle['gambar'],
            'konten' => $randomArticle['konten'],
            'hak_cipta' => $randomArticle['hak_cipta'],
            'created_at' => Carbon::now(),
            'updated_at' => Carbon::now(),
        ];
    }

    // Metode untuk memastikan semua data unik dimasukkan (tidak acak)
    public function sequenceData(array $articlesData)
    {
        return $this->state(new \Illuminate\Support\Fluent(
            $articlesData
        ))->sequence(function ($sequence) use ($articlesData) {
            return $articlesData[$sequence->index % count($articlesData)];
        });
    }
}